...
fn bench_circuit_tick(c: &mut Criterion) {
    ...
    circuit.add_synapse(a, d, 0.4);
    circuit.add_synapse(d, e, 0.6);
    circuit.add_synapse(e, b, -0.2);

    c.bench_function("circuit_tick_4pop_5syn", |b| {
        b.iter(|| {
            let mut c = circuit.clone();
            c.tick(black_box(0.016));
            c
        })
    });
}
...
fn bench_hpa_tick(c: &mut Criterion) {
    ...
    c.bench_function("hpa_tick", |b| {
        b.iter(|| {
            let mut h = hpa.clone();
            h.tick(black_box(0.016));
            h
        })
    });
}

criterion_group!(benches, bench_neurotransmitter_tick, bench_circuit_tick, bench_hpa_tick);
